  ...
  return (
    <SafeAreaLayout withTabBar>
      <main data-testid="achievements-view" className="flex flex-1 flex-col gap-4 p-5">
        <h1 className="text-title text-text-strong">업적</h1>

        {!isLoggedIn ? (
          <div data-testid="achievements-login-gate" className="flex flex-col gap-3">
            <p className="text-body text-subtle">로그인하면 업적을 확인할 수 있어요.</p>
            <LoginBanner />
          </div>
        ) : (
          <div
          ...
          >
            <p className="text-body text-text">업적 기능을 준비하고 있어요.</p>
            <p className="text-caption text-subtle">전북 지도와 업적 목록이 곧 추가됩니다.</p>
          </div>
        )}
      </main>
      <BottomTabBar />
